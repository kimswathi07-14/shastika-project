...
const Shipment = () => {
  ...
  // ✅ Firebase Auth UID — buyer filter-க்கு இதை use பண்றோம்
  ...
  useEffect(() => {
    const unsubscribe = onSnapshot(
      ...
      (snapshot) => {
        ...
        setRealtimeOrders(fetched);
        setLoading(false);
      },
      (error) => {
        console.error('❌ Shipment listener error:', error);
        setLoading(false);
      }
    );
    return () => unsubscribe();
  ...
  // ✅ KEY FIX: buyerId = Firebase Auth UID மூலம் filter
  const myOrders = isAdmin
    ? realtimeOrders
    : realtimeOrders.filter(o => {
        const uid = firebaseUid || currentUser?.id;
        return o.buyerId === uid || o.buyerEmail === currentUser?.email;
      });

  const stats = {
    totalShipments: myOrders.length,
    inTransit: myOrders.filter(o => ['processing', 'shipped', 'transit', 'out_for_delivery'].includes(o.shipmentStatus)).length,
      ...
      {myOrders.length === 0 ? (
        ...
        <div className="space-y-6">
          {myOrders.map(order => {
            ...
            return (
              <div key={order.id} className="premium-card rounded-2xl p-8 space-y-8 hover:shadow-lg transition-all duration-300">
                <div className="flex items-start justify-between gap-4 flex-col md:flex-row">
                  <div className="flex-1">
                    <p className="text-2xl font-bold text-foreground">{order.productName}</p>
                    <p className="text-muted-foreground mt-1">
                      {t('order_id')}: <span className="font-mono font-bold text-foreground">{order.id}</span>
                    </p>
                    <p className="text-muted-foreground mt-1">
                      {t('shipping')}: {order.shippingMethod === 'sea' ? t('shipment_sea_way') : t('shipment_air_way')} ◆{' '}
                      {t('market_type')}: {order.marketType === 'export' ? t('shipment_export') : t('shipment_domestic')}
                    </p>

                    {(order.trackingNumber || order.trackingLink) ? (
                      <div className="mt-4 p-4 bg-primary/5 rounded-lg border border-primary/20">
                        <p className="text-sm font-semibold text-foreground mb-2">{t('shipment_tracking_details')}</p>
                  ...
                  <div className="text-right">
                    ...
                    {order.shipmentUpdatedAt && (
                      <p className="text-xs text-muted-foreground mt-1">
                        Updated: {new Date(order.shipmentUpdatedAt).toLocaleString()}
                      </p>
                    )}
                  </div>
                </div>
                ...
                <div className="relative">
                  <div className="flex items-center justify-between overflow-x-auto">
                    {steps.map((step, i) => {
                      ...
                      return (
                        <div key={step.status} className="flex flex-col items-center relative flex-1 min-w-max px-1">
                          <div className={`w-14 h-14 rounded-2xl flex items-center justify-center border-2 transition-all ${done ? 'gradient-primary text-white border-primary shadow-lg' : 'bg-background text-muted-foreground border-border'}`}>
                            <Icon className="w-6 h-6" />
                          </div>
                          <p className={`text-xs mt-3 text-center font-semibold whitespace-nowrap ${done ? 'text-primary' : 'text-muted-foreground'}`}>
                            {step.label}
                          </p>
                          {!isLast && (
                            <div className={`absolute top-7 -right-1/2 h-1 transition-all ${i < currentIdx ? 'bg-gradient-to-r from-primary to-secondary' : 'bg-border'}`}
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 p-4 bg-muted/30 rounded-lg border border-border">
                  <div>
                    ...
                    <p className="text-lg font-bold text-foreground">{order.quantity} {t('units')}</p>
                  </div>
                  <div>
                    <p className="text-xs text-muted-foreground font-medium">{t('order_date')}</p>
                    <p className="text-lg font-bold text-foreground">{order.orderDate || order.createdAt}</p>
                  </div>
                  <div>
                    <p className="text-xs text-muted-foreground font-medium">{t('amount')}</p>
                    <p className="text-lg font-bold text-primary">₹{Number(order.total || order.amount || 0).toLocaleString()}</p>
                  </div>
                  <div>
                    <p className="text-xs text-muted-foreground font-medium">{t('country')}</p>
                    <p className="text-lg font-bold text-foreground">{order.destinationCountry || order.country || 'India'}</p>
                  </div>
                </div>
              </div>
            );
